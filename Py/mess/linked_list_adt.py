...
class LinkedList:
    ...
    def extend(self, L):
        ...
        if not self.head:
            ...
            return
        node = self.head
        while node.next_node: # loop to the end of the current list
            node = node.next_node
        node.next_node = L.head # append the extended list to the last node of the current list
    ...
    def reverse(self):
        ...
        if not self.head:
            return
        node = self.head.next_node # assign node to the node after head
        self.head.next_node = None # unlink head from the list
        while node: # base condition, node is not None
            next_node = node.next_node # get the next node
