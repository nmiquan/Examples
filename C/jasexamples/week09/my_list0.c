...
struct node *first(struct node *list)
{
    ...
    // but this program implement singly linked list -> no previous pointer
    // thus, the input pointer must alwasy be the first of the list
...
struct node *last(struct node *list)
{
    ...
    struct node *curr = list; // make a copy of the pointer to the list
    while (curr->next != NULL) { // move the copy pointer to the last node
        curr = curr->next;
    }
    return curr;
}
...
struct node *append(int value, struct node *list)
{
    ...
    // input 
    //    value of the node to be append
    //    pointer to the beginning of the list
    ...
    struct node *curr;
    curr = list; // make a copy of the pointer to the beginning of the list
    // scan to last node
    while (curr->next != NULL) {
        curr = curr->next;
    }
    struct node *new = create_node(value, NULL);
    // create_node returns a pointer
    // the pointer is captured to new
    // assign next node of the current pointer to new
    curr->next = new;
    return list;
/*
    if (curr == NULL)
