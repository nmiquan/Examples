...
makeSampleGraph(size_t n)
{
    ...
    g->n = n; /* initializing number of vertices */
    g->v = malloc(sizeof(struct node) * n); /* allocating memory for array of vertices */
    assert(g->v);
...
dfsHelper(struct graph *g, Vertex parent, Vertex child)
{
    ...
    if(g->v[child].parent == VERTEX_NULL) {
        ...
        /*
        neighbor = g->v[child].neighbors[0];
        for(i = 0; neighbor != VERTEX_NULL; i++) {
            neighbor = g->v[child].neighbors[i];
            dfsHelper(g, child, neighbor);
        }
        */
    }
...
main(int argc, char **argv)
{
    ...
    switch(argv[1][0]) {
        ...
        case 'b':
            ...
            break;
        default:
            fprintf(stderr, "%s: unknown action i '%c'\n", argv[0], argv[1][0]);
            return 1;
    }
